...
import com.alphadevs.com.config.KafkaProperties;
...
class WebSocketKafkaVanilaJhKafkaResourceIT {
    ...
    void setup() {
        ...
        WebSocketKafkaVanilaJhKafkaResource kafkaResource = new WebSocketKafkaVanilaJhKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/web-socket-kafka-vanila-jh-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/web-socket-kafka-vanila-jh-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
